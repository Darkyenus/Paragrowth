...
public final class InputStack implements InputProcessor {
    ...
    public void pushIfNotContains(InputProcessor inputProcessor, boolean bubble){
        if (stack.contains(inputProcessor, true)) return;
        stack.add(inputProcessor);
        stackBubble.add(bubble);
    }
    ...
    public void remove(InputProcessor inputProcessor) {
        final int indexOf = stack.indexOf(inputProcessor, true);
        if (indexOf == -1) return; // Already removed, or not there in the first place

        stack.removeIndex(indexOf);
        stackBubble.removeIndex(indexOf);
    }
